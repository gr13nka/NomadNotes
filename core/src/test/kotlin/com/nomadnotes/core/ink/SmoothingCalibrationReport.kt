 ...
 * ```
 *
 * Everything below runs the *real* tuning and pipeline code ([autoSigmaPx], [autoEpsilonPx],
 * [simplify], [smoothStrokeTuned]) rather than a reimplementation of it, so a finding here cannot
 * be an artifact of the report's own arithmetic drifting from the app's. Prints to stdout only,
 * writes nothing to disk; a red run here means the corpus path was wrong or empty, not that
 * smoothing regressed.
 */
class SmoothingCalibrationReport {
    ...
    private fun sweepOutputQuality(strokes: List<Stroke>): Map<Candidate, Map<ShapeBucket, BucketMetrics>> {
        println()
        println("--- output quality (SIGMA_FRACTION_OF_SHAPE x MAX_EPSILON_FRACTION_OF_SHAPE_AUTO), via smoothStrokeTuned ---")

        val sigmaFractions = listOf(0.1f, 0.15f, 0.2f, 0.25f, 0.3f)
        val epsilonFractions = listOf(0.01f, 0.02f, 0.03f, 0.04f, 0.06f)
        val results = LinkedHashMap<Candidate, Map<ShapeBucket, BucketMetrics>>()

        for (sigmaFraction in sigmaFractions) {
            for (epsilonFraction in epsilonFractions) {
                val candidate = Candidate(sigmaFraction, epsilonFraction)
                ...
                for (stroke in strokes) {
                    ...
                    val sigma = autoSigmaPx(distinct, sigmaFraction)
                    val epsilon = autoEpsilonPx(distinct, epsilonFraction)
                    val smoothed = smoothPath(distinct, sigma)
                    val knots = splitLongSpans(simplify(smoothed, epsilon), smoothed, MAX_KNOT_SPAN_PX)
                    if (knots.size < 2) continue
                    val output = smoothStrokeTuned(stroke.points, sigma, epsilon, MAX_KNOT_SPAN_PX)
                    ...
                    // 5 and 8: how much simplification and knot-splicing changed the point count.
                ...
                println("[sigma%=${"%.2f".format(sigmaFraction)} eps%=${"%.2f".format(epsilonFraction)}]")
                for (bucket in ShapeBucket.entries) {
        ...
        if (qualifying.isEmpty()) {
            println(
                "No candidate keeps the small bucket's p99 lostDetail and p99 corner error at or under " +
                    "1.0px across the sigma/epsilon fractions swept. Widen the sweep, or reconsider " +
                    "MIN_SHAPE_SCALE_PX — the small bucket's floor is what both terms fall back on.",
            )
            return
        }
        val best = qualifying.entries.minByOrNull { (_, perBucket) -> normalAndLargeTremor(perBucket) }!!
        println(
            "Selected sigma%=${best.key.sigmaFraction} eps%=${best.key.epsilonFraction}: residual tremor on " +
     ...
     * of Smoothing.kt beyond the [autoSigmaPx]/[autoEpsilonPx]/[simplify]/[smoothStrokeTuned]
     * entry points. */
    ...
    /** The true median distance between consecutive samples of [points], in page pixels — a local
     * copy of the same statistic AUTO no longer needs at runtime, kept here purely to characterize
     * the corpus's sampling rate (see #1 above). */
    private fun medianSampleSpacingPx(points: List<StrokePoint>): Float {
        if (points.size < 2) return 0f
        val spacings = FloatArray(points.size - 1)
        for (i in 1 until points.size) {
            spacings[i - 1] = hypot(points[i].x - points[i - 1].x, points[i].y - points[i - 1].y)
        }
        return percentile(spacings.toList(), 0.50)
    }
    ...
    private data class Candidate(val sigmaFraction: Float, val epsilonFraction: Float)
